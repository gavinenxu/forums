  ...
  <div v-if="asyncDataStatus_ready" class="forum-wrapper">
    <div class="col-full push-top">
      <div class="forum-header">
        ...
        <!-- here pass props through params in router, should use this -->
  ...
  export default {
    ...
    computed: {
      ...
      threads () {
        return this.$store.state.threads.items
          ? Object.values(this.$store.state.threads.items)
    ...
    created () {
      this.fetchForum({id: this.id})
        ...
        .then(threads => {
          if (threads) {
            return Promise.all(threads.map(thread => this.fetchUser({id: thread.userId})))
          }
        })
        .then(() => {
